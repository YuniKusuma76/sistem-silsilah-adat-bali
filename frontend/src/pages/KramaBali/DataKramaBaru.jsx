...
const DataKramaBaru = ({ user }) => {
  ...
  const [berkasKelengkapanParent, setBerkasKelengkapanParent] = useState(null);
    ...
    const fetchData = async () => {
      ...
      try {
        const results = await Promise.allSettled([
          ...
          axiosInstance.get("/krama-bali?mode=personal", { signal: controller.signal }),
          ...
          axiosInstance.get("/perkawinan?mode=personal", { signal: controller.signal }),
          axiosInstance.get("/relasi-krama?mode=public", { signal: controller.signal })
        ]);
  ...
  const handleFileParentChange = (file) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setAlert({
        show: true,
        type: "error",
        message: "Ukuran file terlalu besar! Maksimal ukuran file berkas kelengkapan adalah 2MB."
      });
      return;
    }
    ...
    setBerkasKelengkapanParent(file);
    setPreviewFileNameParent(file.name);
  };

  const handleHapusFileParent = () => {
    setBerkasKelengkapanParent(null);
  ...
  const validateForm = () => {
    ...
    if (parentData.status_diketahui === "Diketahui") {
      if (kramaData.tipe_data === "Keturunan" && !berkasKelengkapanParent) {
        setAlert({ 
          show: true, 
          type: 'error', 
          message: 'Berkas kelengkapan hubungan anak dengan orang tua wajib diunggah!' 
        });
  ...
  const saveKrama = async (e, isConfirmed = false) => {
    ...
    try {
      ...
      // BINDING & ALOKASI AWAL VARIABEL RELASI ORANG TUA
      let ayahId = parentData.selected_ayah_id || null;
      let ibuId = parentData.selected_ibu_id || null;
      let finalPerkawinanId = parentData.selected_perkawinan_id || null;
      let finalSingleParentId = parentData.selected_parent_id || parentData.selected_ayah_id || parentData.selected_ibu_id || null;

      const processParentData = parentData.status_diketahui === "Diketahui";

      // ====================================================================
      // TAHAP 1: MENYIMPAN DATA ORANG TUA (JIKA MANUAL / NEW_ENTRY)
      // ====================================================================
      if (processParentData && parentData.isManual) {
        if (parentData.jenis_pengangkatan === "Pasangan") {
      ...
      if (ayahId) {
        formData.append("ayah_id", String(ayahId));
      }
      if (ibuId) {
        formData.append("ibu_id", String(ibuId));
      }
      ...
      if (Array.isArray(perkawinanlist)) {
        for (const m of perkawinanlist) {
          const statusFormAsli = String(m.status_perkawinan || "").trim().toLowerCase();
          const isCeraiSimultan = statusFormAsli === "cerai" || statusFormAsli === "cerai mati" || statusFormAsli.includes("cerai");
          const isCeraiMatiSimultan = statusFormAsli === "cerai mati" || statusFormAsli.includes("mati");

          if (statusFormAsli !== "belum kawin" && statusFormAsli !== "") {
            ...
            if (isCeraiSimultan) {
              try {
                ...
                await axiosInstance.put(`/perkawinan/cerai/${createdMarriageId}`, payloadCeraiBersih);
              } catch (errError) {
                console.error("Gagal memproses data perceraian simultan:", errError);
              }
            }
          }
        }
      ...
      if (processParentData) {
        ...
        const formRelasiParent = new FormData();

        formRelasiParent.append("anak_id", String(mainId));
        formRelasiParent.append("status_hubungan", parentData.status_hubungan || "Anak Kandung");

        if (parentData.status_hubungan === "Anak Angkat" && parentData.tanggal_pengangkatan) {
          formRelasiParent.append("tanggal_pengangkatan", parentData.tanggal_pengangkatan);
        }

        if (parentData.urutan_lahir) {
          formRelasiParent.append("urutan_lahir", String(parentData.urutan_lahir));
        }
        ...
        if (parentData.jenis_pengangkatan === "Tunggal") { 
          ...
          if (genderParent === "Perempuan") {
            if (finalSingleParentId) {
              formRelasiParent.append("ibu_id", String(finalSingleParentId));
            }
          } else {
            if (finalSingleParentId) {
              formRelasiParent.append("ayah_id", String(finalSingleParentId));
            }
          }
        } else {
          if (ayahId) {
            formRelasiParent.append("ayah_id", String(ayahId));
          }
          if (ibuId) {
            formRelasiParent.append("ibu_id", String(ibuId));
          }
          if (safePerkawinanId) {
            formRelasiParent.append("perkawinan_id", String(safePerkawinanId));
          }
        }

        if (berkasKelengkapanParent) {
          formRelasiParent.append("berkas_kelengkapan", berkasKelengkapanParent);
        }

        await axiosInstance.post("/relasi-krama", formRelasiParent, {
          headers: { "Content-Type": "multipart/form-data" }
        });
      }
      ...
      if (adoptingData.status_pengangkatan === "Ya" || adoptingData.status_pengangkatan === "Mengangkat Anak") {
        if (adoptingData.anak_angkat_id || adoptingData.isAnakManual) {
          ...
          const formRelasiAnak = new FormData();
          formRelasiAnak.append("anak_id", String(finalAnakId));
          formRelasiAnak.append("status_hubungan", "Anak Angkat");

          if (adoptingData.tanggal_pengangkatan_anak) {
            formRelasiAnak.append("tanggal_pengangkatan", adoptingData.tanggal_pengangkatan_anak);
          }

          const perkawinanKramaUtamaId = createdMarriageId || kramaData?.selected_perkawinan_id;

          if (perkawinanKramaUtamaId) {
            formRelasiAnak.append("perkawinan_id", String(perkawinanKramaUtamaId));
          } else {
            if (kramaData.jenis_kelamin === "Laki-laki") {
              formRelasiAnak.append("ayah_id", String(mainId));
            } else {
              formRelasiAnak.append("ibu_id", String(mainId));
            }
          }
          
          if (berkasPengangkatanAnak) {
            formRelasiAnak.append("berkas_kelengkapan", berkasPengangkatanAnak);
          }

          await axiosInstance.post("/relasi-krama", formRelasiAnak, {
            headers: { "Content-Type": "multipart/form-data" }
          });
        }
      }
  ...
  // Helper Form Input
  const isPurusa = perkawinanlist.some((m) => {
    if (m.status_perkawinan === "Belum Kawin") return true;
    if (kramaData.jenis_kelamin === "Laki-laki") {
      return m.jenis_perkawinan === "Biasa" || m.jenis_perkawinan === "Pade Gelahang";
    }
    if (kramaData.jenis_kelamin === "Perempuan") {
      return m.jenis_perkawinan === "Nyentana" || m.jenis_perkawinan === "Pade Gelahang";
    }
    return false;
  }) || perkawinanlist.length === 0;

  const labelPengangkatanAnak = isPurusa ? "IV. Data Pengangkatan Anak" : "III. Data Pengangkatan Anak";
              ...
              <div className="flex gap-3">
                ...
                <div className="space-y-1">
                  <h4 className="text-sm font-bold text-blue-900 uppercase tracking-wide">
                    Pemberitahuan Penting Penginputan Silsilah Adat Bali
                  </h4>
                  <p className="text-xs text-blue-700 leading-relaxed">
                    Untuk menjaga akurasi struktur pohon silsilah keluarga dan menghindari kegagalan relasi basis data krama, disarankan untuk menginputkan data krama secara <strong>berurutan sesuai kronologis nyata (dari generasi tertua/leluhur terlebih dahulu, baru kemudian generasi keturunan)</strong>.
                  </p>
                  <ul className="text-[11px] text-blue-600 list-disc list-inside space-y-0.5 pt-1 italic">
                    ...
                    <li>Pastikan krama utama pada form I adalah krama dari desa adat yang sama ketika mendaftarakan perkawinan adat.</li>
                  </ul>
                </div>
              </div>
            </div>
                    ...
                    <div key={m.id_temp} className={styles.cardSection}>
                      ...
                      {m.status_perkawinan !== "Belum Kawin" && (
                        <div className={`${styles.popupInput} animate-fade-in`}>
                              ...
                              <input
                                ...
                                value={openDropdownIndex === index ? (searchPasangan[index] || "") 
                                  ...
                                  : (() => {
                                      ...
                                      if (!pasanganTerpilih) return "";
                                      const isDraft = pasanganTerpilih.status_verifikasi === "Draft";
                                      return `${pasanganTerpilih.nama_lengkap}${
                                ...
                                onChange={(e) => {
                                  if (!kramaData.jenis_kelamin) {
                                    ...
                                    setAlert({
                                      show: true,
                                      type: "warning",
                                      message: "Harap pilih jenis kelamin krama pada Form 1 terlebih dahulu sebelum memilih pasangan."
                                    });
                                    return;
                                  }
                          ...
                          {m.pasangan_id && !m.isPasanganBaru && (() => {
                            const pasanganTerpilih = kramaList.find(k => String(k.id) === String(m.pasangan_id));
                            if (!pasanganTerpilih) return null;
                ...
                <div className={styles.cardSection}>
                  ...
                  {parentData.status_diketahui === "Diketahui" && (
                    <div className={`${styles.popupInput} animate-fade-in`}>
                              ...
                              <div className="relative">
                                <input
                                  ...
                                  value={isDropdownOpen ? searchOrangTuaTerm 
                                    ...
                                    : (() => {
                                        const selectedParent = kramaList.find((k) => String(k.id) === String(parentData.selected_parent_id));
                                        if (!selectedParent) return "";
                                        const isDraft = selectedParent.status_verifikasi === "Draft";
                                        return `${selectedParent.nama_lengkap}${
                                          isDraft ? " (DRAFT - MENUNGGU VERIFIKASI)" : ""
                                        }`;
                                      })()
                                  }
                      ...
                      {/* Berkas Kelengkapan */}
                      <div className="flex flex-col space-y-1.5 animate-fade-in pt-2">
                        <label className={styles.labelInputSelect}>
                          {parentData.status_hubungan === "Anak Angkat" 
                            ? "Berkas Kelengkapan Anak Angkat" 
                            : "Berkas Kelengkapan Anak Kandung"}
                          {kramaData.tipe_data === "Keturunan" ? (
