    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const results = await Promise.allSettled([
          axiosInstance.get(`/relasi-krama/${realId}`),
        ...
        if (resRelasi) {
          ...
          // mengambil data lama anak, ayah, dan ibu
          ...
          if (apakahModeAdopsi) {
            ...
            setParentData({
              ...
              jenis_pengangkatan: resRelasi.perkawinan_id ? "Pasangan" : "Tunggal",
              tanggal_pengangkatan: resRelasi.tanggal_pengangkatan ? resRelasi.tanggal_pengangkatan.substring(0, 10) : "",
              selected_perkawinan_id: resRelasi.perkawinan_id || "",
        ...
        console.error("Gagal memuat entri detail relasi:", error);
        setAlert({ 
          show: true, 
          type: 'error', 
    ...
    try {
      ...
      if (adoptingData.status_pengangkatan === "Ya" || adoptingData.status_pengangkatan === "Mengangkat Anak") {
        if (adoptingData.anak_angkat_id || adoptingData.isAnakManual) {
          ...
          const isAdminRole = user?.role === "Super Admin" || user?.role === "Admin Desa";
          ...
          let payloadA = { 
            ...
            status_verifikasi: isAdminRole ? "Disetujui" : "Draft",
          ...
          if (perkawinanAktifKrama) {
            ...
            if (kramaData.jenis_kelamin === "Laki-laki") {
              payloadA.ayah_id = safeInt(anchorKramaId);
              payloadA.ibu_id = null;
            } else {
              payloadA.ibu_id = safeInt(anchorKramaId);
              payloadA.ayah_id = null;
            }
            payloadA.perkawinan_id = null;
          }
          
          console.log("Mengirim Payload A (Adopsi Edit) ke Server:", payloadA);
          await axiosInstance.put(`/relasi-krama/${realId}`, payloadA);
        }
      }
      // ====================================================================
      ...
      // ====================================================================
        ...
        if (parentData.isManual) {
          ...
          if (
            ...
            // PERBAIKAN: Perbaikan Logika Terbalik Ayah
            if (ayahId) {
              await axiosInstance.put(`/krama-bali/${ayahId}`, payloadAyah);
            } else {
            ...
            // PERBAIKAN: Perbaikan Logika Terbalik Ibu
            if (ibuId) {
              await axiosInstance.put(`/krama-bali/${ibuId}`, payloadIbu);
            } else {
            ...
            // Mendaftarkan perkawinan orang tua baru jika belum ada
          ...
          // Skenario 2: Relasi dengan orang tua tunggal secara manual
          else {
            ...
            // PERBAIKAN: Perbaikan Logika Terbalik Single Parent
            if (finalSingleParentId) {
              await axiosInstance.put(`/krama-bali/${finalSingleParentId}`, payloadSingle);
            } else {
        ...
        console.log("Mengirim Payload R Edit ke Server:", payloadR);
        await axiosInstance.put(`/relasi-krama/${realId}`, payloadR);
      }

      // Kembali ke halaman silsilah krama bali
      navigate(-1, { 
        state: { successMessage: 'Perubahan relasi silsilah keluarga berhasil disimpan!' } 
      });
    } catch (error) {
      console.error("=== DIAGNOSTIK ERROR ===", error.response?.data);
      setAlert({ 
        show: true, 
        type: 'error', 
        message: error.response?.data?.message || 'Terjadi kesalahan pada sistem. Periksa kembali validitas form.' 
      });
      window.scrollTo(0, 0);
    } finally { 
      setIsLoading(false); 
      ...
      <nav className={styles.navbar}>
        <div className={styles.navLeft}>
          <h2 className={styles.navTitle}>
            Memperbarui Data Relasi Orang Tua
          </h2>
          <p className={styles.navSubtitle}>
            Perbarui data relasi orang tua dengan data baru yang sebenarnya dan sah
          </p>
        </div>
