...
const DataKramaEditRelasi = ({ user }) => {
  ...
  const [existingBerkasPath, setExistingBerkasPath] = useState(null);
      ...
      try {
        ...
        if (resRelasi) {
          ...
          if (entitasKrama) {
            ...
            setKramaData({
              nomor_pendaftaran: profilLengkapKrama?.nomor_pendaftaran || entitasKrama.nomor_pendaftaran || "",
            ...
            if (entitasKrama.foto_profile) {
              setPreviewFoto(`${SUPABASE_STORAGE_URL}${entitasKrama.foto_profile}`);
            }
          ...
          let dataPerubahanRaw = resRelasi.data_perubahan;

          if (dataPerubahanRaw && dataPerubahanRaw.data_perubahan) {
            dataPerubahanRaw = dataPerubahanRaw.data_perubahan;
          }

          const berkasTersimpan = dataPerubahanRaw?.berkas_kelengkapan || resRelasi.berkas_kelengkapan;
          const catatanDraftRelasi = dataPerubahanRaw?.catatan_update || "";

          if (berkasTersimpan) {
            setExistingBerkasPath(berkasTersimpan);
            const namaFileAsli = berkasTersimpan.split('/').pop().split('?')[0];
            const namaBersih = namaFileAsli.includes('-') 
              ? namaFileAsli.substring(namaFileAsli.indexOf('-') + 1) 
              : namaFileAsli;

            if (apakahModeAdopsi) {
              setPreviewFileNameAnak(namaBersih);
            } else {
              setPreviewFileNameParent(namaBersih);
            }
          }

          if (apakahModeAdopsi) {
            setActualAnakId(idAnakKrama ? parseInt(idAnakKrama) : null);
            setAdoptingData({
              status_pengangkatan: "Mengangkat Anak",
              anak_angkat_id: idAnakKrama || "",
            ...
            setActualAnakId(kramaJangkarId ? parseInt(kramaJangkarId) : null);
            ...
            if (!perkawinanIdFinal && resRelasi.ayah_id && resRelasi.ibu_id) {
              ...
              if (matchCocok) {
                perkawinanIdFinal = matchCocok.id;
              }
            }
            ...
            setParentData({
              ...
              catatan_update: catatanDraftRelasi,
  ...
  const handleHapusFileParent = () => {
    ...
    setExistingBerkasPath(null);
    if (berkasParentRef.current) {
      berkasParentRef.current.value = "";
    }
  };
  ...
  const handleBack = () => {
    ...
    const entitasKrama = currentRelasiRaw?.anak || currentRelasiRaw?.ayah || currentRelasiRaw?.ibu;

    if (idAnakKrama && entitasKrama) {
      const slug = createSlug(
        entitasKrama.nama_lengkap || kramaData?.nama_lengkap, 
        entitasKrama.tipe_data || kramaData?.tipe_data, 
        idAnakKrama
      );
      
      navigate(`/krama-bali/my-data/detail/${slug}`, { 
        replace: true 
      });
    } else {
  ...
  const validateForm = () => {
    ...
    if (parentData.status_diketahui === "Diketahui") {
      const hasBerkasParent = berkasKelengkapanParent || previewFileNameParent || existingBerkasPath;
      if (kramaData.tipe_data === "Keturunan" && !hasBerkasParent) {
        setAlert({ 
          show: true, 
          type: 'error', 
    ...
    if (adoptingData.status_pengangkatan === "Mengangkat Anak") {
      ...
      const hasBerkasAdopting = berkasPengangkatanAnak || existingBerkasPath;

      if (tipeDataAnak === "Keturunan" && !hasBerkasAdopting) {
        setAlert({ 
          show: true, 
          type: 'error', 
          message: 'Berkas kelengkapan hubungan anak dengan orang tua wajib diunggah!' 
  ...
  const saveKrama = async (e, isConfirmed = false) => {
    ...
    try {
      setIsLoading(true);
      let payloadR = null;
      let targetAnakIdForFetch = actualAnakId || safeInt(anchorKramaId);
      ...
      if (adoptingData.status_pengangkatan === "Ya" || adoptingData.status_pengangkatan === "Mengangkat Anak") {
        if (adoptingData.anak_angkat_id || adoptingData.isAnakManual) {
          ...
          targetAnakIdForFetch = safeInt(finalAnakId);
          ...
          payloadR = { 
            ...
            berkas_kelengkapan: existingBerkasPath || null,
            ...
            urutan_lahir: parentData.urutan_lahir ? parseInt(parentData.urutan_lahir, 10) : null,
      ...
      // ====================================================================
        ...
        let safePerkawinanId = finalPerkawinanId && !isNaN(finalPerkawinanId) ? parseInt(finalPerkawinanId, 10) : null;
        targetAnakIdForFetch = safeInt(anchorKramaId);

        payloadR = { 
          anak_id: safeInt(anchorKramaId), 
      ...
      // ====================================================================
      // PROSES EKSEKUSI PENYIMPANAN KE BACKEND
      // ====================================================================
      if (payloadR) {
        let idRelasiTarget = currentRelasiRaw?.id;

        if (!idRelasiTarget && targetAnakIdForFetch) {
          const resFind = await axiosInstance.get(`/relasi-krama/anak/${targetAnakIdForFetch}`);
          const relasiAmbil = resFind.data?.data || resFind.data;
        ...
        Object.keys(payloadR).forEach((key) => {
          ...
          if (val === null || val === undefined) {
            formDataR.append(key, "");
          } else {
            formDataR.append(key, val);
          }
        });

        const fileToUpload = berkasKelengkapanParent || berkasPengangkatanAnak;
        
        if (fileToUpload && fileToUpload instanceof File) {
          formDataR.append("berkas_kelengkapan", fileToUpload);
        }
      ...
      setAlert({ 
        show: true, 
        type: 'error', 
        message: error.response?.data?.message || error.message || 'Terjadi kesalahan pada sistem saat memperbarui data relasi krama.' 
      });
      window.scrollTo(0, 0);
    } finally { 
      setIsLoading(false); 
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-bold text-amber-900 uppercase tracking-wider">
                  Pemberitahuan Perubahan Data Relasi Krama
                </h4>
                ...
                <p className="text-[11px] text-amber-700 mt-2 font-semibold">
                  <strong>PENTING:</strong> <i>Disarankan mengajukan penghapusan data relasi krama kepada Admin Desa, jika adanya kesalahan pada input data struktural, seperti nama orang tua, status hubungan, dan struktur orang tua.</i>
                </p>
              </div>
            </div>
          </div>
                ...
                {parentData.status_diketahui === "Diketahui" && (
                  <div className={`${styles.popupInput} animate-fade-in`}>
                    ...
                    <div className="flex flex-col space-y-4">
                      {(parentData.status_hubungan === "Anak Kandung" || (parentData.status_hubungan === "Anak Angkat" && parentData.jenis_pengangkatan === "Pasangan")) && parentData.jenis_pengangkatan !== "Tunggal" ? (
                        ...
                        <>
                          ...
                          {(() => {
                            const activeParentId = parentData.selected_parent_id || parentData.selected_ayah_id || parentData.selected_ibu_id;
                            if (!activeParentId || parentData.isManual) return null;

                            let ortuTunggalTerpilih = kramaList.find(
                              k => String(k.id) === String(activeParentId)
                            );

                            if (!ortuTunggalTerpilih && currentRelasiRaw) {
                              if (currentRelasiRaw.ayah_id && String(currentRelasiRaw.ayah_id) === String(activeParentId)) {
                                ortuTunggalTerpilih = currentRelasiRaw.ayah;
                              } else if (currentRelasiRaw.ibu_id && String(currentRelasiRaw.ibu_id) === String(activeParentId)) {
                                ortuTunggalTerpilih = currentRelasiRaw.ibu;
                              }
                            }
                    ...
                    <div className="flex flex-col space-y-1.5 text-sm">
                      ...
                      <textarea 
                        name="catatan_update" 
                        value={parentData.catatan_update || ""}
                        onChange={handleParentChange} 
                        rows={3}
                        className={styles.inputText}
                ...
                {adoptingData.status_pengangkatan === "Mengangkat Anak" && (
                  <div className={`${styles.popupInput} animate-fade-in`}>
                    ...
                    {(() => {
                      ...
                      const isInputAnakRequired = isDokumenWajib && !berkasPengangkatanAnak && !previewFileNameAnak;
