...
const DataKramaPerceraian = ({ user }) => {
  ...
  const saveKrama = async (e, isConfirmed = false) => {
    ...
    try {
      ...
      for (const m of perkawinanlist) {
        ...
        if (!targetPasanganId) {
          throw new Error("Data identitas pasangan tidak ditemukan.");
        }

        const payloadPutBackend = {
          tipe_update: "PERCERAIAN", 
          ...
          tanggal_event: safeDate(m.tanggal_cerai), 
          status_perkawinan: m.status_perkawinan,
          pihak_meninggal: m.status_perkawinan === "Cerai Mati" ? (m.pihak_meninggal || "Pasangan") : null,
          pilihan_predana: m.status_perkawinan === "Cerai Mati" ? (m.pilihan_predana || "Kembali ke Asal") : (m.pilihan_predana || "Kembali ke Asal"),
          catatan_update: m.catatan_update
        };

        const resUpdate = await axiosInstance.put(`/perkawinan/cerai/${m.id_asli_db}`, payloadPutBackend);
        console.log(resUpdate.data);
      }

      navigate(`/krama-bali/my-data/detail/${mainId}`, { 
        state: { successMessage: 'Data perceraian perkawinan adat berhasil disimpan!' },
        replace: true
      });
      ...
      <nav className={styles.navbar}>
        <div className={styles.navLeft}>
          <h2 className={styles.navTitle}>
            Pengajuan Perceraian Perkawinan Adat
          </h2>
          <p className={styles.navSubtitle}>
            Lengkapi data perceraian perkawinan adat dengan data baru yang sebenarnya dan sah
          </p>
        </div>
            ...
            <section className={styles.section}>
              <div className={styles.sectionContent}>
                ...
                {kramaData.jenis_kelamin === "Laki-laki" && (
                  <button type="button" onClick={tambahBarisPerkawinan} className={styles.btnAddIstri} disabled={true}>
            ...
            <div className="p-6">
              ...
              <div className="text-center">
                <h3 className="text-lg font-bold text-gray-900 mb-2">
                  Konfirmasi Pengajuan Perceraian
                </h3>
                <p className="text-sm text-gray-600 leading-relaxed">
                  Apakah Anda yakin mengajukan perceraian pada data perkawinan krama utama ini dan data yang dituliskan sudah benar, sah, dan sesuai dengan awig-awig/pararem desa adat?
                </p>
                <p className={styles.noteConf}>
                  * Perceraian pada perkawinan adat Purusa-Predana ini akan langsung mempengaruhi diagram pohon silsilah keluarga dan membutuhkan proses verifikasi ulang jika terdapat kesalahan input data.
                </p>
              </div>
...
export default DataKramaPerceraian;